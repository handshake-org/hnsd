...
static void hsk_blake2b_set_lastnode( hsk_blake2b_ctx *S )
{
  S->f[1] = (uint64_t)-1;
}

/* Some helper functions, not necessarily useful */
static int hsk_blake2b_is_lastblock( const hsk_blake2b_ctx *S )
{
  return S->f[0] != 0;
}

static void hsk_blake2b_set_lastblock( hsk_blake2b_ctx *S )
{
  if( S->last_node ) hsk_blake2b_set_lastnode( S );

  S->f[0] = (uint64_t)-1;
}

static void hsk_blake2b_increment_counter( hsk_blake2b_ctx *S, const uint64_t inc )
{
  S->t[0] += inc;
  S->t[1] += ( S->t[0] < inc );
}

static void hsk_blake2b_init0( hsk_blake2b_ctx *S )
{
  size_t i;
  memset( S, 0, sizeof( hsk_blake2b_ctx ) );

  for( i = 0; i < 8; ++i ) S->h[i] = hsk_blake2b_IV[i];
}
...
int hsk_blake2b_init_key( hsk_blake2b_ctx *S, size_t outlen, const void *key, size_t keylen )
{
  ...
  if ( !key || !keylen || keylen > HSK_BLAKE2B_KEYBYTES ) return -1;
  ...
  if( hsk_blake2b_init_param( S, P ) < 0 ) return -1;

  {
    ...
    secure_zero_memory( block, HSK_BLAKE2B_BLOCKBYTES ); /* Burn the key from stack */
  }
  return 0;
}
...
static void hsk_blake2b_compress( hsk_blake2b_ctx *S, const uint8_t block[HSK_BLAKE2B_BLOCKBYTES] )
{
  uint64_t m[16];
  uint64_t v[16];
  size_t i;
...
int hsk_blake2b_update( hsk_blake2b_ctx *S, const void *pin, size_t inlen )
{
  ...
  if( inlen > 0 )
  {
    ...
    if( inlen > fill )
    {
      ...
      hsk_blake2b_compress( S, S->buf ); /* Compress */
      in += fill; inlen -= fill;
...
int hsk_blake2b_final( hsk_blake2b_ctx *S, void *out, size_t outlen )
{
  ...
  memset( S->buf + S->buflen, 0, HSK_BLAKE2B_BLOCKBYTES - S->buflen ); /* Padding */
...
int hsk_blake2b( void *out, size_t outlen, const void *in, size_t inlen, const void *key, size_t keylen )
{
  ...
  if( keylen > HSK_BLAKE2B_KEYBYTES ) return -1;

  if( keylen > 0 )
  {
    if( hsk_blake2b_init_key( S, outlen, key, keylen ) < 0 ) return -1;
  }
  else
  {
    if( hsk_blake2b_init( S, outlen ) < 0 ) return -1;
  }
