...
static void blake2b_set_lastnode( blake2b_ctx *S )
{
  S->f[1] = (uint64_t)-1;
}

/* Some helper functions, not necessarily useful */
static int blake2b_is_lastblock( const blake2b_ctx *S )
{
  return S->f[0] != 0;
}
...
static void blake2b_increment_counter( blake2b_ctx *S, const uint64_t inc )
{
  S->t[0] += inc;
  S->t[1] += ( S->t[0] < inc );
}
...
static void blake2b_compress( blake2b_ctx *S, const uint8_t block[BLAKE2B_BLOCKBYTES] )
{
  uint64_t m[16];
  uint64_t v[16];
  size_t i;
...
int blake2b_update( blake2b_ctx *S, const void *pin, size_t inlen )
{
